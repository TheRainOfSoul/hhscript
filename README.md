...
- **Батарея и железо** — модель, серийник, CPU/память, циклы и состояние
  батареи (`system_profiler`), диск и SMART. Только чтение.
- **Автозагрузка** — элементы входа (Login Items) и агенты
  `~/Library/LaunchAgents` / системные, с отключением пользовательского агента
  (переименование в `.disabled`).
- **Обслуживание и очистка** — обзор «что занимает место», чистка кэшей и
  корзины (с подтверждением), `brew cleanup`, сброс DNS-кэша, `purge`. Действия
  с правами — через окно ввода пароля (`osascript … with administrator privileges`).
...
- **Система:** Информация о ПК · MAS (активация) · Лёгкая чистка
  ...
  **Сканер сети** (находит камеры/устройства: вендор по MAC, проба портов камер
  с пометкой RTSP, веб-интерфейс по клику, сортировка колонок, живой прогресс без
  зависания окна) с **ONVIF-поиском** (WS-Discovery — находит камеры, даже молчащие
  на ping, вендор/модель и точный RTSP/snapshot-URL берёт **от самой камеры** по
  логину/паролю — не угадывая пути) и **встроенным просмотром камеры** — кадр по
  HTTP-снапшоту прямо в окне (ONVIF/Dahua/Hikvision-пресеты, Digest-авторизация,
  автообновление ~1 к/с, **ничего не качая** — Windows рисует JPEG сам); для камер
  без веб-снапшота — кнопка «RTSP-ссылка» (в буфер + в VLC, если установлен). ONVIF
  доступен и в CLI (Сетевые утилиты → пункт 9). Всё inline, без тяжёлых внешних утилит.
