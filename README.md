...
- **Система:** Информация о ПК · MAS (активация) · Лёгкая чистка
  ...
  **Сканер сети** (находит камеры/устройства: вендор по MAC, проба портов камер
  ...
  зависания окна) с **встроенным просмотром камеры** — кадр по HTTP-снапшоту
  прямо в окне (Dahua/Hikvision-пресеты, Digest-авторизация, автообновление ~1 к/с,
  **ничего не качая** — Windows рисует JPEG сам); для камер без веб-снапшота —
  кнопка «RTSP-ссылка» (в буфер + в VLC, если установлен). Всё inline, без тяжёлых
  внешних утилит.
